...
export function WelcomeMessage({
  ...
  const [currentView, setCurrentView] = useState<"welcome" | "cards">("cards");
        ...
        <div className="absolute flex flex-col justify-center align-middle">
          <div className="flex flex-col items-start gap-2 mb-[50px]">
            ...
            <p className="text-white text-[16px]">
              TU NUEVO ASISTENTE KITCHEN AID
            </p>
          </div>
          <img
            ...
            className={`size-[120px] mx-auto bg-[#c41230] rounded-full transition-all duration-300 ${
              isLoadingAI
      ...
      {currentView === "cards" && (
        <div className=" my-auto w-full max-w-6xl px-6 grid grid-rows-[20%_60%_20%] gap-4 items-center h-[85dvh]">
          <div className="flex flex-col items-start gap-2 mb-[50px] w-full">
            <img
              src="/img/WelcomeSpanish.svg"
              className="w-[70%] mx-auto"
            ></img>
            <p className="text-white text-[14px] mx-auto">
              TU NUEVO ASISTENTE KITCHEN AID
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <button
              onClick={
                !isLoadingAI && !isLoadingSkip ? handleNextClick : undefined
              }
              disabled={isLoadingAI || isLoadingSkip}
              className={`w-full px-6 py-3 bg-[#c41230] text-white font-semibold rounded-full transition-all duration-300 shadow-lg ${
                isLoadingAI || isLoadingSkip
                  ? "opacity-70 cursor-wait"
                  : "hover:bg-[#a00f26] cursor-pointer"
              }`}
            >
              {isLoadingAI ? (
                <span className="flex items-center justify-center gap-2">
