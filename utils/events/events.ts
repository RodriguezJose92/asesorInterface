...
class Events {
  ...
  multimedia(productInfo: ProductInfo | null = null) {
    ...
    const setterProductInfo =
      useImageCarouselStore.getState().setterProductInfo;
...
export function registerEventListeners() {
  ...
  eventBus.subscribe<ProductInfo>(EventTypes.SHOW_3D, (event: any) =>
    events.view3D(event.payload)
  );
  eventBus.subscribe<ProductInfo>(EventTypes.SHOW_MULTIMEDIA, (event: any) =>
    events.multimedia(event.payload)
  );
  eventBus.subscribe<ProductInfo>(EventTypes.SHOW_IMAGES, (event: any) =>
    events.viewImages(event.payload)
  );
  eventBus.subscribe<ProductInfo>(EventTypes.SHOW_VIDEO, (event: any) =>
    events.viewVideo(event.payload)
  );
  eventBus.subscribe<ProductInfo>(EventTypes.SHOW_AR, (event: any) =>
    events.viewAR(event.payload)
  );
  eventBus.subscribe(EventTypes.CLOSE_CAROUSEL, () =>
    events.closeCarousel()
  );
  eventBus.subscribe(EventTypes.HIDE_3D, () =>
    events.hiddenQuickQuestions()
  );
  eventBus.subscribe(EventTypes.HIDE_MULTIMEDIA, () =>
    events.hiddenQuickQuestions()
  );
  eventBus.subscribe(EventTypes.UI_UPDATE, () =>
    events.hiddenMessagePolitices()
  );
