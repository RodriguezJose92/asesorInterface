...
export const multilingualGreetings: MultilingualGreetings = {
  es: {
    greeting:
      "Hola, soy Kit-AI. Estoy aquí para guiarte, ya sea hablando o escribiendo. Juntos haremos realidad la cocina que imaginas.",
    followUp:
      "Puedes preguntarme sobre cualquier electrodoméstico que necesites.",
    languageDetected:
      "He detectado que tu navegador está en español. Hablaré contigo en español.",
    languageSwitch: "Perfecto, ahora hablaré contigo en español.",
  },
  en: {
    greeting:
      "Hi, I’m Kit-AI. I’m here to guide you, whether you prefer speaking or typing. Together, we’ll bring your dream kitchen to life.",
    followUp: "You can ask me about any appliance you need.",
    languageDetected:
      "I've detected your browser is in English. I'll speak with you in English.",
    languageSwitch: "Perfect, I'll now speak with you in English.",
  },
  fr: {
    greeting:
      "Bonjour ! Je suis votre assistant électroménager. Comment puis-je vous aider aujourd'hui ?",
    followUp:
      "Vous pouvez me demander des informations sur n'importe quel appareil électroménager.",
    languageDetected:
      "J'ai détecté que votre navigateur est en français. Je parlerai avec vous en français.",
    languageSwitch: "Parfait, je vais maintenant vous parler en français.",
  },
  de: {
    greeting:
      "Hallo! Ich bin Ihr Haushaltsgeräte-Assistent. Wie kann ich Ihnen heute helfen?",
    followUp:
      "Sie können mich nach jedem Haushaltsgerät fragen, das Sie benötigen.",
    languageDetected:
      "Ich habe erkannt, dass Ihr Browser auf Deutsch eingestellt ist. Ich werde mit Ihnen auf Deutsch sprechen.",
    languageSwitch: "Perfekt, ich werde jetzt mit Ihnen auf Deutsch sprechen.",
  },
  it: {
    greeting:
      "Ciao! Sono il tuo assistente per elettrodomestici. Come posso aiutarti oggi?",
    followUp:
      "Puoi chiedermi informazioni su qualsiasi elettrodomestico di cui hai bisogno.",
    languageDetected:
      "Ho rilevato che il tuo browser è in italiano. Parlerò con te in italiano.",
    languageSwitch: "Perfetto, ora parlerò con te in italiano.",
  },
  pt: {
    greeting:
      "Olá! Sou seu assistente de eletrodomésticos. Como posso ajudá-lo hoje?",
    followUp:
      "Você pode me perguntar sobre qualquer eletrodoméstico que precisar.",
    languageDetected:
      "Detectei que seu navegador está em português. Falarei com você em português.",
    languageSwitch: "Perfeito, agora falarei com você em português.",
  },
  ja: {
    greeting:
      "こんにちは！私はあなたの家電アシスタントです。今日はどのようにお手伝いできますか？",
    followUp: "必要な家電について何でもお聞きください。",
    languageDetected:
      "ブラウザが日本語に設定されていることを検出しました。日本語でお話しします。",
    languageSwitch: "完璧です。これから日本語でお話しします。",
  },
  ko: {
    greeting:
      "안녕하세요! 저는 가전제품 어시스턴트입니다. 오늘 어떻게 도와드릴까요?",
    followUp: "필요한 가전제품에 대해 무엇이든 물어보세요.",
    languageDetected:
      "브라우저가 한국어로 설정되어 있음을 감지했습니다. 한국어로 대화하겠습니다.",
...
export const getLanguageDetectedMessage = (languageCode: string): string => {
  const greeting = multilingualGreetings[languageCode];
  return greeting
    ? greeting.languageDetected
    : multilingualGreetings.en.languageDetected;
};

export const getLanguageSwitchMessage = (languageCode: string): string => {
  const greeting = multilingualGreetings[languageCode];
  return greeting
    ? greeting.languageSwitch
    : multilingualGreetings.en.languageSwitch;
};
...
export const languageDetectionPatterns = {
  ...
  ko: [/[\uAC00-\uD7AF]/, /\b(안녕하세요|안녕|도움|필요|원해|찾고있어)\b/i],
  zh: [
    /[\u4E00-\u9FFF]/,
